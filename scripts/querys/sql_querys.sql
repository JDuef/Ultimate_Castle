...
/* Abfragen Einheiten/Angriffe */
SELECT einheit.einheit_id, einheit.einheit_name, attribute.attribut_bezeichnung
from attribute
inner join einheit on attribute.attribut_id = einheit.einheit_id;

SELECT ANGRIFF.ANGRIFF_ID, ANGRIFF.ANGRIFF_STATUS, KRIEGSBEUTE.KRIEGSBEUTE_ANZAHL
FROM ANGRIFF
FULL OUTER JOIN KRIEGSBEUTE
ON ANGRIFF.ANGRIFF_ID = KRIEGSBEUTE.ANGRIFF_ID
ORDER BY ANGRIFF_ID;

SELECT EINHEIT_GRUPPE.GRUPPE_ID, EINHEIT.EINHEIT_NAME, EINHEIT_GRUPPE.GRUPPE_ANZAHL
FROM EINHEIT
FULL OUTER JOIN EINHEIT_GRUPPE
ON EINHEIT.EINHEIT_ID = EINHEIT_GRUPPE.GRUPPE_ID
ORDER BY EINHEIT_GRUPPE.GRUPPE_ID;
